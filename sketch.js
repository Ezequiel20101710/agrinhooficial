...
// Balão de fala (texto alinhado no centro do balão)
function desenharBalaoDeFala(x, y, texto) {
  fill(255);
  stroke(0);
  rect(x - 120, y - 60, 240, 60, 10); // Retângulo principal do balão
  triangle(x, y, x - 10, y + 20, x + 10, y + 20); // Ponteiro do balão
  noStroke();
  fill(0);
  textSize(14);
  textAlign(CENTER, CENTER); // Centraliza horizontal e verticalmente o ponto de âncora do texto

  // Calcula o centro vertical do balão
  let centroYBalao = (y - 60) + (60 / 2); // Início do balão (y-60) + metade da altura (30)

  // Desenha o texto no centro do balão
  text(texto, x, centroYBalao);
}
